...
import com.tencent.qcloud.timchat.utils.richtext.MSimpleText;
...
public abstract class Message {
    ...
    public RelativeLayout getBubbleView2(ChatAdapter.ViewHolder viewHolder, String msgContent) {
        viewHolder.systemMessage.setVisibility(hasTime ? View.VISIBLE : View.GONE);
        viewHolder.systemMessage.setText(TimeUtil.getChatTimeStr(message.timestamp()));
        showDesc(viewHolder);
        viewHolder.leftPanel.setVisibility(View.VISIBLE);
        ...
        if (message.getConversation().getType() == TIMConversationType.Group) {
            ...
            if (isSelf()) {
                name = "我";
                MSimpleText simpleText = MSimpleText.create(ImApplication.getContext(), name + "  :  " + msgContent)
                        .first(name).textColor(R.color.common_fff176)
                        .first(msgContent).textColor(R.color.common_fff176);
                simpleText.linkify(viewHolder.sender);
                viewHolder.sender.setText(simpleText);
            } else {
                MSimpleText simpleText = MSimpleText.create(ImApplication.getContext(), name + "  :  " + msgContent)
                        .first(name).textColor(R.color.common_1097d0)
                        .first(msgContent).textColor(R.color.common_white);
                simpleText.linkify(viewHolder.sender);
                viewHolder.sender.setText(simpleText);
            }
